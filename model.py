from layers import (conv3d, deconv3d, normalizationlayer, crop_and_concat, resnet_Add,
                        weight_init, bias_variable, save_images)
...
def conv_bn_relu_drop(x, kernal, phase, drop, image_z=None, height=None, width=None, scope=None):
    with tf.name_scope(scope):
        W = weight_init(shape=kernal, n_inputs=kernal[0] * kernal[1] * kernal[2] * kernal[3],
...
def deconv_relu(x, kernal, samefeture=False, scope=None):
    with tf.name_scope(scope):
        W = weight_init(shape=kernal, n_inputs=kernal[0] * kernal[1] * kernal[2] * kernal[-1],
...
class Vnet3dModule(object):
    ...
    def train(self, train_images, train_lanbels, model_path, logs_path, learning_rate,
              dropout_conv=0.8, train_epochs=30, batch_size=1):
        ...
        saver = tf.compat.v1.train.Saver(tf.all_variables(), max_to_keep=10)

        tf.compat.v1.summary.scalar("loss", self.cost)
        tf.compat.v1.summary.scalar("accuracy", self.accuracy)
        merged_summary_op = tf.compat.v1.summary.merge_all()
        sess = tf.compat.v1.InteractiveSession(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
        summary_writer = tf.compat.v1.summary.FileWriter(logs_path, graph=tf.get_default_graph())
        sess.run(init)
	...
	#train_epochs = train_images.shape[0] * train_epochs
		...
		#save_images(result, [4, 8], path=logs_path + 'predict_%d_epoch.png' % (i))
                ...
                # train on batch
            	summary = sess.run([train_op, merged_summary_op], feed_dict={self.X: batch_xs,
                                                                            self.Y_gt: batch_ys,
                                                                            self.lr: learning_rate,
                                                                            self.phase: 1,
                                                                            self.drop: dropout_conv})
